// personalCenter/address/amendAddress/amendAddress.js
...
Page({
   ...
   */
	...
	deleteAdd:function(e){
		wx.request({
			url: app.globalData.localhost+'/deleteAddress',
			data:{
				uAddressId:this.data.id
			},
			success(e){
				wx.navigateBack({
					delta:1
	...
	onLoad: function (options) {
		var that=this;
		var id = options.uAddressId
		console.log(id)
		wx.request({
			url: app.globalData.localhost + '/getAddressInfo?uAddressId='+id,
			method: 'get',
			header: {
			...
			success(e){
				...
				that.setData({
					id:id,
					name:e.data.data.ua_name,
					phone:e.data.data.ua_phone,
					rProvince:e.data.data.province_name,
					pro_Id: e.data.data.province_id,
					rCity:e.data.data.city_name,
					cit_Id:e.data.data.city_id,
					rArea:e.data.data.area_name,
					are_Id:e.data.data.area_id,
					concreteAdd:e.data.data.ua_address,
					def:e.data.data.ua_is_default
				})
				console.log(that.data.def)
			}
		})
	},
	...
	save: function (e) {
		...
		console.log(d)
		wx.request({
			url: app.globalData.localhost + '/updateAddress',
			data: {
				uAddressId:that.data.id,
				uaPhone: that.data.phone,
				uaName: that.data.name,
				provinceId: that.data.pro_Id,
				cityId: that.data.cit_Id,
				areaId: that.data.are_Id,
				uaAddress: that.data.concreteAdd,
				uaIsDefault: d
			},
			success(e) {
				console.log(e)
